...
A total of `r nrow(ms)` individual `r Streamname` observations were recorded.   The number of `r Streamname` observations recorded by individual stations is as follows:
...
After processing, the number of unified observations meeting quality criteria as a percentage of individual observations was `r rows_to_process` from `r nrow(ms)` individual station observations (`r sprintf("%.1f",100*rows_to_process/nrow(ms))`)%.  
...
The number of individual station observations over time is as follows:
...
The number of unified observations over time is as follows:

```{r echo=FALSE, fig.height = 5, fig.extra='style="float:left"'}
source(paste(PlotDir,"stream_plot_timeline_unified.r",sep="/"),local=TRUE)
box(which="plot",lty="solid")
```
...
The following graphs show the distribution of velocities and magnitudes for the `r Streamname` is shown below:
