...
Streamname    = ""
Solpeak       = 139.212097	
SelectStart   = "2016/01/02 18:18"
SelectEnd     = "2016/12/31 08:08"
```
...
In `r SelectYr` a total of `r length(unique(mo$X_ID1))` cameras cntributed data to the UKMON repository.  Together, these cameras recorded `r nrow(mo)` individual meteor observations.  The number of recorded observations by station is as follows:
...
A total of `r nrow(mu[mu$X_mag <= -4,])` fireballs were observed in the reporting period ('r SelectYr`).  Here we define a fireball as a meteor event with a magnitude of -4 or less.  A breakdown by month and by shower is shown below.
...
The brightest (top 10) fireballs observed during `r SelectYr` were:
